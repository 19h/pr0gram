(function(root, factory) {
	...
	else root.GibberishAES = factory()
})(this, function() {
	var Nr = 14,
		Nk = 8,
		Decrypt = false,
		...
		padBlock = function(byteArr) {
			...
			for (i = 0; i < byteArr.length; i++) array[i] = byteArr[i];
			return array
		},
		block2s = function(block, lastBlock) {
			...
			if (lastBlock) {
				...
				for (i = 0; i < 16 - padding; i++) string += String.fromCharCode(block[i])
			} else
		...
		a2h = function(numArr) {
			var string = "",
				i;
			for (i = 0; i <
				numArr.length; i++) string += (numArr[i] < 16 ? "0" : "") + numArr[i].toString(16);
			return string
		},
		h2a = function(s) {
			var ret = [];
		...
		randArr = function(num) {
			var result = [],
				i;
			for (i = 0; i < num; i++) result = result.concat(Math.floor(Math.random() * 256));
			return result
		},
		...
		rawEncrypt = function(plaintext,
			key, iv) {
			key = expandKey(key);
			var numBlocks = Math.ceil(plaintext.length / 16),
				blocks = [],
				i, cipherBlocks = [];
			for (i = 0; i < numBlocks; i++) blocks[i] = padBlock(plaintext.slice(i * 16, i * 16 + 16));
		...
		rawDecrypt = function(cryptArr, key, iv, binary) {
			...
			var numBlocks = cryptArr.length / 16,
				...
				i, plainBlocks = [],
				string = "";
			for (i = 0; i < numBlocks; i++) cipherBlocks.push(cryptArr.slice(i * 16, (i + 1) * 16));
			for (i = cipherBlocks.length - 1; i >= 0; i--) {
				plainBlocks[i] = decryptBlock(cipherBlocks[i], key);
				plainBlocks[i] = i === 0 ? xorBlocks(plainBlocks[i], iv) : xorBlocks(plainBlocks[i], cipherBlocks[i - 1])
			}
			for (i = 0; i < numBlocks - 1; i++) string += block2s(plainBlocks[i]);
		...
		subBytes = function(state) {
			var S = Decrypt ?
				...
				temp = [],
				i;
			for (i = 0; i < 16; i++) temp[i] = S[state[i]];
			return temp
		},
		shiftRows = function(state) {
			var temp = [],
				...
				i;
			for (i = 0; i < 16; i++) temp[i] = state[shiftBy[i]];
			return temp
		},
			...
			if (!Decrypt)
				for (c = 0; c < 4; c++) {
					...
					t[2 + c * 4] = state[c * 4] ^ state[1 + c * 4] ^
						G2X[state[2 + c * 4]] ^ G3X[state[3 + c * 4]];
		...
		addRoundKey = function(state, words, round) {
			var temp = [],
				i;
			for (i = 0; i < 16; i++) temp[i] = state[i] ^ words[round][i];
			return temp
		},
		xorBlocks = function(block1, block2) {
			var temp = [],
				i;
			for (i = 0; i < 16; i++) temp[i] = block1[i] ^ block2[i];
			return temp
		},
		expandKey = function(key) {
			...
			for (i = Nk; i < 4 * (Nr + 1); i++) {
				...
				for (t = 0; t < 4; t++) w[i][t] = w[i - Nk][t] ^
					temp[t]
			}
			for (i = 0; i < Nr + 1; i++) {
				flat[i] = [];
				for (j = 0; j < 4; j++) flat[i].push(w[i * 4 + j][0], w[i * 4 + j][1], w[i * 4 + j][2], w[i * 4 + j][3])
			}
			return flat
		},
		subWord = function(w) {
		...
		strhex = function(str, size) {
			var i, ret = [];
			for (i = 0; i < str.length; i += size) ret[i / size] = parseInt(str.substr(i, size), 16);
			return ret
		},
		invertArr = function(arr) {
			var i, ret = [];
			for (i = 0; i < arr.length; i++) ret[arr[i]] = i;
			return ret
		},
		Gxx = function(a,
			b) {
		...
		MD5 = function(numArr) {
			...
			function gg(a, b, c, d, x, s, ac) {
				a = addUnsigned(a, addUnsigned(addUnsigned(g(b,
					c, d), x), ac));
				return addUnsigned(rotateLeft(a, s), b)
			}
		...
		encString = function(plaintext, key, iv) {
			...
			key = s2a(key);
			for (i = key.length; i < 32; i++) key[i] = 0;
			if (iv === undefined);
			else {
				iv = s2a(iv);
				for (i = iv.length; i < 16; i++) iv[i] = 0
			}
			var ct = rawEncrypt(plaintext,
				key, iv);
			var ret = [iv];
			for (i = 0; i < ct.length; i++) ret[ret.length] = ct[i];
		...
		Base64 = function() {
			var _chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/",
				...
				encode = function(b, withBreaks) {
					...
					for (i = 0; i < b.length * 16; i++) flatArr.push(b[Math.floor(i / 16)][i % 16]);
					for (i = 0; i < flatArr.length; i = i + 3) {
						b64 += chars[flatArr[i] >> 2];
						b64 += chars[(flatArr[i] & 3) << 4 | flatArr[i + 1] >> 4];
						if (flatArr[i + 1] !== undefined) b64 += chars[(flatArr[i + 1] & 15) << 2 | flatArr[i + 2] >> 6];
						else b64 += "="; if (flatArr[i + 2] !== undefined) b64 += chars[flatArr[i + 2] & 63];
						else b64 += "="
					}
					broken_b64 = b64.slice(0, 64) + "";
					for (i = 1; i < Math.ceil(b64.length / 64); i++) broken_b64 += b64.slice(i * 64, i * 64 + 64) + (Math.ceil(b64.length / 64) ===
						i + 1 ? "" : "");
					return broken_b64
				},
			...
			if (typeof Array.indexOf ===
				"function") _chars = chars;
			return {
				"encode": encode,
				"decode": decode
			}
